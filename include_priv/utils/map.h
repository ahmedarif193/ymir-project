...
public:
    ...
    Node(std::size_t _level, const ValueType& _val){
        ...
        size_t _total_size = sizeof(Node*)*(_temp_level);
        memset(_forward_links, '\0', _total_size);
        _value = new pair<const K, M>(_val);
        _prev = NULL;
    }
...
M& map<K, M>::operator[](const K& _key){
    Node<K, M> *_temp_head = find_at_bottom(_key);
    if(_temp_head == NULL){
        map<K, M>::Iterator _iter = insert(make_pair(_key, M())).first;
...
template <typename K, typename M>
pair<typename map<K, M>::Iterator, bool> map<K, M>::insert(const pair<const K, M>& _val){
